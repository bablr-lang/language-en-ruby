...
export const mixin = (Base) =>
  class RubyCallGrammar extends Base {
    constructor() {
      super();
      this.attributes = new Map([
        ...(this.attributes || []),
        ['UnaryCall', { power: undefined, position: undefined }],
        ['MethodCall', { power: undefined }],
        ['CommaCall', { power: undefined }],
        ['BinaryCall', { power: undefined }],
        ['AssignmentCall', { power: undefined }],
        ['MemberCall', { power: undefined }],
      ]);
    }

    *Call({ props: { power = 34 }, ctx }) {
      let res;
      if (power >= 2 && (yield match(re`/\g[[.]/`))) {
        res = yield eat(m`<MemberCall />`, o({ power }));
      }
      if (!res && power >= 6) {
        res = yield eatMatch(m`<MethodCall />`, o({ power }));
      }
      if (!res && power >= 8 && (yield match(re`/[+-~!]/`))) {
        res = yield eat(m`<UnaryCall />`, o({ power }));
      }
      if (!res && power >= 12) {
        res = yield eatMatch(m`<BinaryCall />`, o({ power }));
      }
      if (!res && power >= 32) {
        if ((res = yield eatMatch(m`<AssignmentCall/>`, o({ power })))) {
        }
      }
      if (!res && power >= 34) {
        res = yield eatMatch(m`<CommaCall />`, o({ power }));
      }

      const opPower = res && ctx.sourceTextFor(res.node.get('sigilToken'));
    }

    *UnaryCall({ props: { power }, s }) {
      let ownPower = 0;
      let op;
      if (!s.holding) {
    ...
    *AssignmentCall() {
      ...
      yield eat(m`right+$: <_Expression />`, o({ power: 30 }));
    }
