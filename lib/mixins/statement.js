  ...
  class RubyStatementGrammar extends Base {
    ...
    *Statements({ ctx, s }) {
      ...
      while (yield match(re`/./s`)) {
        stmt = yield eat(m`body[]+: <_Statement />`);
        if (
        ...
        newline = trivia && ctx.sourceTextFor(trivia.node).includes('\n');

        if (!(stmt.node.get('endToken') || newline)) break;
      }
    }
    ...
    *ForStatement() {
      ...
      if (!(yield eatMatch(m`init: <VariableDeclaration />`))) {
        yield eatMatch(m`init+$: <_Expression />`);
      }
    ...
    *ForInStatement() {
      ...
      if (!(yield eatMatch(m`left: <VariableDeclaration />`))) {
        yield eatMatch(m`left: <_Expression />`);
      }
